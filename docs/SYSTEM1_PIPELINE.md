...
1. **字符层**：Unicode NFKC 归一（顺带把 ①→1、Ⅰ→I、全角括号→半角）；删零宽字符/BOM/控制字符；各种空格（不间断空格、全角空格）→ 普通空格。
2. **全角→半角**：只动 ASCII（０→0、Ａ→A、英文 `()%:-`）；**中文标点（，。；："") 一律保留**。
3. **标点统一**：异体引号/破折号/省略号归一；中文标点本身不动。
4. **OCR 修复**（仅 OCR 来源的块，黑白名单见下）。
5. **删页眉页脚页码**：③ 按位置标的候选 + ④ 跨页重复检测，**两条都中才删**。
6. **修断行**：行尾不是句末标点（。！？）且下行不是新块/新编号开头 → 拼回；中文直接拼、英文连字符断词（compa-\nny）拼回。
7. **压缩空白**：连续空格→1 个、连续空行→1 个、行首尾去空格。
