...
**这个系统是干嘛的？** 你给它一份劳动合同，它告诉你：哪几条违法或对你不公平、违反了哪条法律、该怎么改。站在劳动者一边，但只讲法律事实、不偏不向。

**它长什么样？** 把它想象成一家"审合同的公司"，分 6 个部门；一份合同从上往下走一遍：

| 层 | 大白话 | 好比公司里的 | 干的活 |
|----|--------|------------|--------|
| L1 接入层 | 大门 | 前台 | 收你上传的合同，最后把报告给你（网页 / 命令行 / 供别的软件调用）|
| L2 编排层 | 调度室 | 经理 | 决定这活怎么干——走固定流水线，还是让 AI 助手自己边想边做 |
| L3 能力层 | 各车间 | 干活的工人 | 真本事：拆合同、找风险、查法条、复核、出报告 |
| L4 模型层 | 工具机器 | 工人手里的设备 | 各种 AI 模型（大语言模型、识图、OCR…），被上面的车间调用 |
| L5 数据与知识层 | 资料室 | 档案库 | 法律条文、案例、测试数据——系统的"长期记忆" |
| L6 基础设施层 | 水电地基 | 后勤 | 服务器、日志、脱敏、部署——让上面 5 层能跑、可信、好查 |

**一份合同的旅程（举例）**：你在网页上传 `合同.pdf`（L1）→ 调度室说"这是审合同，走流水线"（L2）→ 车间把它拆成一条条条款、逐条找违法点、查对应法条、复核、拼成报告（L3，过程中用 L4 的模型、查 L5 的法律）→ 报告回到网页给你（L1）。

**两个老出现的词，先说清楚**：
- **workflow（流水线）**：固定几步、按顺序走、每次一样。适合"审合同 → 出报告"这种确定的活。这是 **System 1**。
- **agent loop（会自己想的助手）**：没有固定步骤，AI 自己决定下一步干嘛、要不要查资料、什么时候答完。适合"用户随便提问"这种开放的活。这是 **System 2**（P5 才做）。

下面是正式的分层描述，术语会多一点，但都对应上面这张表。
