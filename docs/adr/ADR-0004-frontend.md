...
1. **演示**：面试时展示 agent 工作过程（上传合同 → 显示风险标注 → 引用法条 → 给建议）
...
- 前端不是项目核心价值；面试官关心的是后端架构和 eval 严谨度
...
| HTML + HTMX | TODO（建议：视觉不够现代，2026 面试场景下不加分） |
...
TODO（建议）：
- Streamlit "ML demo 感" — 面试官可能认为不像生产产品
...
- **怎么知道这个决策对了**：TODO（建议）：P5b demo 视频拍出来流畅，面试官未对前端提出负面问题
